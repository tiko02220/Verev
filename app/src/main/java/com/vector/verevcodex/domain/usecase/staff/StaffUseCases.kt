...
class UpdateStaffMemberUseCase(private val repository: StaffRepository) {
    suspend operator fun invoke(staffId: String, draft: StaffMemberDraft) = repository.updateStaffMember(staffId, draft)
}

class RemoveStaffMemberUseCase(private val repository: StaffRepository) {
    suspend operator fun invoke(staffId: String) = repository.removeStaffMember(staffId)
}
